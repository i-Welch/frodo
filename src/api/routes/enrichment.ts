...
export const enrichmentRoutes = new Elysia({ prefix: '/api/v1/users' })
  ...
  // -----------------------------------------------------------------------
  .post('/:id/enrich', async ({ params, tenant, apiKey, userId, environment, set }) => {
  ...
  // -----------------------------------------------------------------------
  .post('/:id/enrich/:module', async ({ params, tenant, apiKey, userId, environment, set }) => {
    // Verify tenant-user link
    const link = await getLink(tenant.tenantId, params.id);
    if (!link) {
      set.status = 404;
